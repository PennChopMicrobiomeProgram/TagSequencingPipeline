...
### mapping file path
#mapping_file_fp <- file.path(data_files_dir, "PLEASE FILL ME IN WITH MAPPING FILE NAME")
...
s <- merge(s, faith, by="SampleID", all.x=T)
```
...
```{r}
#run_dates <- unique(s$run_start_date)
#run_dates <- c("20171103", "20170622")

#apply(run_dates, function(x) paste())

run_date <- paste(unique(s$run_start_date), collapse = " ")
#run_data <- paste(run_date[5:6], run_date[], collapse="")
...
s <- s %>%
  ...
  mutate(KeepLabel = factor(ifelse(Keep, "Keep", "Discard"))) %>%
  mutate(isControl = grepl("geneblock|freewater|extractemptywell", SampleID, ignore.case = TRUE))
