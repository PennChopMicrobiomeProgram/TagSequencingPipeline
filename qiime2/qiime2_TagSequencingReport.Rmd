...
rmarkdown::render('qiime2_TagSequencingReport.Rmd',output_file = paste(Sys.Date(),'_Basic16SReport.pdf', sep=''))
```
...
#install.packages(c("dplyr", "magrittr", "qiimer", "pander", "ape", "vegan", "ggplot2", "gplots", "pheatmap", "tidyr", "usedist", "readr", "tibble", "lme4"))
library(dplyr)
library(magrittr)
library(qiimer)
library(pander)
...
### mapping file path
mapping_file_fp <- list.files(file.path(root_dir, "metadata"), pattern = ".tsv", full.names = TRUE)
...
s <- read_qiime_mapping_file(mapping_file_fp) %>%
  rename_all(recode, SubjectID = "subject_id", SampleType = "sample_type", HostSpecies = "host_species") %>% ##rename columns
...
```{r histogram, fig.width=6, fig.height=4, warning=F}
ggplot(s, aes(x=Read_Counts)) +
    geom_histogram(aes(fill=sample_type), binwidth=1000) +
...
prop_cut <- 0.01
satu_limit <- 0.4
heatmap_fp <- file.path(root_dir, "otu_heatmap.pdf")
...
cols_to_keep <- s[s$Keep & !s$isControl,"SampleID"]
...
props_toPlot %>%
  ggplot(aes(x=DNA_concentration, y=Prop)) +
    ...
    labs(x="DNA_concentration (ng/uL)", color="SampleType",
...
```{r richness, fig.height=2.5}
alpha_measure <- "richness"

s %>% filter(Keep & !isControl) %>%
...
```{r uu pcoa, fig.width=6, fig.height=5, echo=FALSE, warning=FALSE}
s_toPlot <- s %>%
  filter(Keep) %>%
  filter(!isControl)
...
```{r wu pcoa, fig.width=6, fig.height=5, echo=FALSE, warning=FALSE}
s_toPlot <- s %>%
  filter(Keep) %>%
  filter(!isControl)
...
```{r}
s %>% 
  select(SampleID, Read_Counts, library_concentration_ng_ul, Keep) %>%
  rename(library_ng_ul = "library_concentration_ng_ul") %>%
  arrange(-Read_Counts) %>% 
  pander()
```
